...
namespace UserMagaement.Controllers
{
    ...
    public class AuthenticateController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("register-Audit")]
        public async Task<IActionResult> RegisterAudit(RegisterModel model)
        {
            var userExists = await userManager.FindByNameAsync(model.Email);
            if (userExists != null)
                return StatusCode(StatusCodes.Status500InternalServerError, new Response { Status = "Error", Message = "User already exists!" });
            ...
            if (await roleManager.RoleExistsAsync(UserRoles.Audit))
            {
                await userManager.AddToRoleAsync(user, UserRoles.Audit);
            }
